...
from .serializers import PetListingSerializer, ListingImageSerializer
...
class Listing(ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if age:
            if age == 'baby':
                queryset = queryset.filter(age__gte=0)
            elif age == 'juvenile':
                queryset = queryset.filter(age__gte=1, age__lte=3)
            elif age == 'adult':
                queryset = queryset.filter(age__gte=4, age__lte=11)
            elif age == 'senior':
                queryset = queryset.filter(age__gte=12)
        if gender:
        ...
        if self.request.user.is_authenticated:
            if self.request.User.user_type == 'shelter':
                ...
                for image_data in images:
                    image_serializer = ListingImageSerializer(data={'listing': self.request.User.pk, 'image': image_data})
                    image_serializer.is_valid(raise_exception=True)
                    image = image_serializer.save()
                    serializer.instance.images.add(image)
            else:
                return HttpResponse(status=403)
        else:
            return HttpResponse(status=401)

        #shelter = get_object_or_404(User, pk=1)
        #serializer.save(shelter=shelter)
        #images = self.request.data.getlist('images')
        #for image_data in images:
        #    image_serializer = ListingImageSerializer(data={'listing': shelter.pk, 'image': image_data})
        #    image_serializer.is_valid(raise_exception=True)
        #    image = image_serializer.save()
        #    serializer.instance.images.add(image)
